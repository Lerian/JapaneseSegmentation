...
def word_segmentation(model, sentence):
    ...
    sentence = "W"+sentence
    #print sentence
    printTest(sentence, "------ trigrams ---")
    for i in range(len(sentence)-2):
        trigram = sentence[i:i+3]
        printTest(sentence, trigram)
    ...
    printTest(sentence, "---sentence")
    printTest(sentence, sentence)
    printTest(sentence, "---lattice")
    printTest(sentence, lattice)
    # Initialisation
    V = []
    ...
    for i in range(1, len(observations)):
        ...
        printTest(sentence, "---V")
        printTest(sentence, V)
        best_V = []
        best_V.append(V[-1])
        printTest(sentence,"---best_V")
        printTest(sentence, best_V)
        highest_prob = best_V[0][0]
        printTest(sentence,"---highest_prob")
        printTest(sentence,highest_prob)
        for j in range(len(V)-2,-1,-1):
            printTest(sentence, "--- j")
            printTest(sentence, j)
        ...
        for prob, stack in best_V:
            ...
            printTest(sentence, "--- prob")
            printTest(sentence, prob)
            printTest(sentence, "--- stack")
            printTest(sentence, stack)
